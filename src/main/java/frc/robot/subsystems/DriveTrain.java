...
public class DriveTrain extends SubsystemBase {
  ...
  private SwerveDriveOdometry odometry;
  // private SwerveDriveOdometry odometry = new SwerveDriveOdometry(kinematics, Rotation2d.fromDegrees(-m_gyro.getAngle()),
  //     getModulePositions());

  private PIDController vx = new PIDController(2.8 , 0 , 0.4);//1.2,0.3,0.1 1 0 0.2
  private PIDController vy = new PIDController(2.8 , 0 , 0.4);//vx同vy//3.1,0,0.6
  ...
  // private final SwerveModule[] swerveModules = { moduleA, moduleB, moduleC, moduleD };
  ...
  // vz.enableContinuousInput(-180, 180);
    ...
    ModuleConfig moduleConfig = new ModuleConfig(
        ConsSwerve.kWheelDiameter/2.0,                          // Wheel radius in meters (4 in / 2)
  ...
  public void periodic() {
    odometry.update(Rotation2d.fromDegrees(-m_gyro.getAngle()), getModulePositions());
...
public void autoDrive(ChassisSpeeds chassisSpeeds) {
  ...
  public void drive(double vxMetersPerSecond, double vyMetersPerSecond, double omegaRadiansPerSecond, Rotation2d latestEstimatedPose) {
  ...
  public Rotation2d getDiretion() {
    return m_gyro.getRotation2d();//.plus(Rotation2d.fromDegrees(-90));//.plus(Rotation2d.fromDegrees(-90))
  }
...
public void driveApriltag(double x, double y, ConsAuto.Position position) {
  ...
  if (errorX < 0.02) xOutput = 0;
  if (errorY < 0.02) yOutput = 0;
  ...
  if(Math.abs(angleError)<1.5) zOutput=0;
