...
public class Lift extends SubsystemBase {
  ...
  private final CANcoder armSensor = new CANcoder(5); 

  private double armSensorOffset = 0;
  ...
  public Lift() {
    ...
    // CANcoder Offset
    armSensorOffset = armSensor.getPosition().getValueAsDouble();
  ...
  public double getArmPosition() {
    return getArmPositionFromSensor() - armSensorOffset;
  }

  public void zeroArmPosition() {
    armSensorOffset = getArmPositionFromSensor();
  }
  // ========== Arm PID 控制 ==========
  ...
  private void updateArmPIDControl() {
    if (useArmPID) {
...
public boolean isArmAtTarget() {
  ...
  public void setPose(ConsLift.Pose pose) {
    ...
    if (pose.armFirst) {
      setArmPosition(arm);
      if (Math.abs(arm - getArmPosition()) <= range) setLiftPosition(lift);
    } else {
      setLiftPosition(lift);
      if (Math.abs(lift - getLiftPosition()) <= range) setArmPosition(arm);
    }
  }
